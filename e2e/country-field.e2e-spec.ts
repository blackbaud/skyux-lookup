  ...
  function runTests(): void {
    ...
    it(
      ...
      async (done) => {
        ...
        expect('#screenshot-country-field-populated').toMatchBaselineScreenshot(done, {
          screenshotName: getScreenshotName('country-field-populated-disabled-xs')
        });
      }
    );
    ...
    it('should match previous screenshot while in an input box', async (done) => {
      await SkyHostBrowser.scrollTo('#screenshot-country-field-empty-input-box');

      expect('#screenshot-country-field-empty-input-box').toMatchBaselineScreenshot(done, {
        screenshotName: getScreenshotName('country-field-empty-input-box')
      });
    });

    it(
      'should match previous screenshot while in an input box and disabled and empty',
      async (done) => {
        await SkyHostBrowser.scrollTo('#screenshot-country-field-empty-input-box');
        await element(by.css('#disable-toggle')).click();

        expect('#screenshot-country-field-empty-input-box').toMatchBaselineScreenshot(done, {
          screenshotName: getScreenshotName('country-field-empty-input-box-disabled')
        });
      }
    );
    ...
    it('should match previous screenshot while in an input box with no label', async (done) => {
      await SkyHostBrowser.scrollTo('#screenshot-country-field-populated-input-box');

      expect('#screenshot-country-field-populated-input-box').toMatchBaselineScreenshot(done, {
    ...
    it(
      ...
      async (done) => {
        ...
        expect('#screenshot-country-field-populated-input-box').toMatchBaselineScreenshot(done, {
          screenshotName: getScreenshotName('country-field-populated-input-box-disabled')
        });
      }
    );
